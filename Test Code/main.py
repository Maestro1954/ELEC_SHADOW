...
def capSaveWindow(event=0):
    ...
    if not timerArmed:
        ...
        button2.config( text="Open File", command=open)
        button0.focus()
        button2.focus()
...
def saveCapture(event=0):
    ...
    screenshot = "C:/Users/images/screenshot_{}.png".format(img_counter)
...
def open():
    global my_image
    mainWindow.filename = filedialog.askopenfilename(initialdir="/Users/images", title="Select a File", filetypes=(("png files","*.png"),("all files","*.*")))
    my_label= Label(mainWindow, text=mainWindow.filename).pack()
    my_image = ImageTk.PhotoImage(Image.open(mainWindow.filename))
...
mainWindow.bind('<Escape>',
                lambda e: mainWindow.quit())  # .bind('<Escape>', ...) makes the esc key close the main window
...
button5 = tk.Button(mainWindow, text = "Open File", command=open)
...
button2.place(bordermode=tk.INSIDE, relx=0.85, rely=0.7, anchor=tk.CENTER, width=150, height=50)
button1.place(bordermode=tk.INSIDE, relx=0.85, rely=0.8, anchor=tk.CENTER, width=150, height=50)
button5.place(bordermode=tk.INSIDE, relx=0.85, rely=0.6, anchor=tk.CENTER, width=150, height=50)
